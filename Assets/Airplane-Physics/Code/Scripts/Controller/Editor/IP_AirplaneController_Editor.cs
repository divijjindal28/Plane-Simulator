    ...
    public class IP_AirplaneController_Editor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Get Airplane Components", GUILayout.Height(35)))
            {
                ...
                targetController.controlSurfaces = FindAllControlSurfaces().ToList<IP_Airplane_ControlSurfaces>();
            }
            if (GUILayout.Button("Create Airplane preset", GUILayout.Height(35)))
            {
                string filePath = EditorUtility.SaveFilePanel("Save Airplane Presets",
                    "Assets", "New_Airplane_Preset", "asset");
        ...
        IP_Airplane_ControlSurfaces[] FindAllControlSurfaces()
        {
            IP_Airplane_ControlSurfaces[] controlSurfaces = new IP_Airplane_ControlSurfaces[0];
            if (targetController)
            {
                controlSurfaces = targetController.transform.GetComponentsInChildren<IP_Airplane_ControlSurfaces>(true);
            }
        ...
        void SaveAirplanePreset(string aPath) {
            if (targetController && !string.IsNullOrEmpty(aPath)) {
                ...
                if (targetController.characteristics) {
                    ...
                    newPreset.rbLerpSpeed = targetController.characteristics.lerpSpeed;
